...
def forward(conn, target, ID):
    ...
    try:
        while status[0]:
            ...
            if data:
                proxy = must_send(target, buffersize, 8, ID, data)
            else: break
    ...
    if hostname and port:
        if(method.lower() == b"connect"):
            try:
                ...
                result = must_send(IP, buffersize, 8, ID, f"HTTP/{version} 200 Connection Established".encode())
            ...
            if TCPs.get(ID): TCPs[ID].send(data)
                ...
                try:
                    ...
                    if result: must_send(IP, buffersize, 8, ID, result)
                    proxy.close()
